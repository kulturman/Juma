...
export class updateTorrentTable1654472168610 implements MigrationInterface {
  name = 'updateTorrentTable1654472168610';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE \`torrents\` CHANGE \`status\` \`status\` set ('QUEING', 'QUEUED', 'STARTED', 'COMPLETED', 'SEEDING') NOT NULL`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE \`torrents\` CHANGE \`status\` \`status\` set ('QUEUED', 'STARTED', 'COMPLETED', 'SEEDING') NOT NULL`,
    );
    await queryRunner.query(
      `ALTER TABLE \`torrents\` ADD CONSTRAINT \`FK_bc0ea4b12ab974dc41cc069ef6e\` FOREIGN KEY (\`user_id\`) REFERENCES \`users\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`,
    );
    ...
    await queryRunner.query(
      `ALTER TABLE \`torrents\` DROP COLUMN \`torrent_name\``,
    );
  }
}
